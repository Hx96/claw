...
### 2026-07-31（tokenless降级最终修复）
- **根因确认**：微信 iLink context_token 24小时过期（官方确认），过期后 ret=-2 "prepare failed"
- **代码修复**：api.ts 加 ret检查 + send.ts sendWithFallback（带token发送→失败降级tokenless重试）
- **结论**：从04月到07月反复踩了10+次的cron投递问题，根因就是context_token过期

### 2026-08-04
- 大盘日报(07:00)超时失败（240s），手动重新触发
- 用户报体重87.3kg，里程碑目标87.0kg差0.3kg
- 记忆整理：07-29~08-03均为常规cron日，无重大交互

### 2026-07-21（cron投递彻底修复）
- **根因找到**：cron prompt里让AI手动调message工具 → 隔离会话投递不可靠 → 微信侧丢失
- **修复方案**：清除所有prompt中手动message指令 + delivery.mode=announce（系统层投递）
- **三层保障**：announce投递 + bestEffort=false（投递失败=任务失败）+ failureAlert.after=1（失败即告警微信）
- **验证通过**：20:12手动触发4个任务，全部delivered=True，用户确认收到
- **教训（最终版）**：cron隔离会话的投递必须由系统层announce负责，AI只生成内容。AI在隔离会话调message工具看似成功（返回messageId），但消息可能被微信折叠/吞掉。这条教训从04-11到07-21反复踩了10次
- **07-18 ~ 07-20**: 微信推送反复未收到（6/30、7/11、7/18、7/20），旧方案用AI手动调message + messageId验证，实际不可靠。07-21确认唯一可靠路径是系统层announce投递
